...
import EPayModal from '../components/modal/EpayModal';
...
function CheckOut() {
  ...
  //Called when user picks eSewa or Khalti from modal
  const handleEPaySelect = async (walletId) => {
    setShowEPayModal(false);
    await handlePlaceOrder(walletId);
  };

  const handlePlaceOrder = async (walletId = null) => {
    try {
  ...
  //  Handle "Pay & Place Order" button click
  const handlePayButtonClick = () => {
    if (paymentMethod === "online") {
      setShowEPayModal(true); // open modal to pick wallet
    } else {
      handlePlaceOrder();
    }
  };
  ...
  return (
    <div className='w-full min-h-screen bg-[#fff9f6] flex flex-col items-center justify-center gap-5'>
      ...
      {/* E-Pay Modal */}
      <EPayModal
        isOpen={showEPayModal}
        onClose={() => { setShowEPayModal(false); setIsProcessing(false); }}
        onSelect={handleEPaySelect}
