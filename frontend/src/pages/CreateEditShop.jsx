...
function CreateEditShop() {
  ...
  const {currentCity, currentProvince, currentAddress} = useSelector(state=>state.user);

  const [name, setName] = useState(myShopData?.name || "");
  const [address, setAddress] = useState(myShopData?.address || currentAddress);
  const [city, setCity] = useState(myShopData?.city || currentCity);
  const [province, setProvince] = useState(myShopData?.province ||currentProvince);
      ...
      <div className='max-w-lg w-full bg-white shadow-xl rounded-2xl p-8 border border-orange-100'>
        ...
        <form className='space-y-5' onSubmit={handleSubmit}>
          ...
          <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
            ...
            <input type="text" placeholder='Province' className='w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500' onChange={(e)=>setProvince(e.target.value)} value={province}/>
            </div>
